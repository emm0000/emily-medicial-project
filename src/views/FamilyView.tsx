...
export const FamilyView = ({ currentUser }: { currentUser: UserProfile }) => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, (payload) => {
        setMessages((prev) => {
          ...
          // Siren audio playback and global overlays are centralized in App.tsx
  ...
  const downloadPhoto = async (url: string, title?: string) => {
    try {
      const filename = title ? `${title.replace(/\s+/g, '_')}.jpg` : `family_photo_${Date.now()}.jpg`;
      ...
      // External URL - fetch as blob to bypass CORS download restriction if possible,
      // fallback to opening in new window if CORS blocks
      try {
      ...
      alert('無法下載此相片，已為您在新分頁中開啟，您可以在該分頁手動儲存相片！');
      window.open(url, '_blank');
    }
  };
          ...
          {messages.length > 0 ? messages.map((msg, i) => {
            ...
            return (
              <div key={i} className={`flex items-start gap-4 ${isMe ? 'flex-row-reverse' : ''} ${isSOS ? 'w-full justify-center px-4 my-2' : ''}`}>
                <img 
                  className={`w-16 h-16 rounded-full border-2 object-cover shadow-md transition-all ${
                    isSOS 
                      ? 'border-error animate-pulse scale-110 shadow-red-200 ring-4 ring-red-100 flex-shrink-0' 
                      : 'border-primary flex-shrink-0'
                  }`} 
                  src={senderAvatar} 
                  alt="" 
                />
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-center justify-center w-full mt-2">
              {selectedTitle && (
                <div className="bg-black/60 px-6 py-3 rounded-full backdrop-blur-sm shadow-lg text-white text-center">
                  <p className="text-xl sm:text-2xl font-extrabold tracking-wide">{selectedTitle}</p>
                </div>
              )}
              <button 
                onClick={() => downloadPhoto(selectedImage, selectedTitle)}
                className="bg-primary hover:bg-primary/95 text-white px-6 py-3 rounded-full font-bold flex items-center gap-2 shadow-lg hover:scale-105 active:scale-95 transition-all text-lg cursor-pointer border-none"
              >
                📥 下載相片
              </button>
            </div>
